...
class CourseTableConversionViewModel @Inject constructor(
    ...
    // 移除了未使用的 courseTableRepository 和 appSettingsRepository
    ...
    // UI 事件通道，用于发送一次性副作用（如启动文件选择器，显示 Toast）
    ...
    fun handleFileImport(tableId: String, inputStream: InputStream) {
        viewModelScope.launch {
            ...
            try {
                ...
                val importModel =
                    Json.decodeFromString<CourseImportExport.CourseTableImportModel>(jsonString)
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (icsContent != null) {
                    ...
                    _events.send(ConversionEvent.ShowMessage(message)) // 修复了这里：之前只定义了变量没有 send
                } else {
...
sealed class ConversionEvent {
    ...
    data class LaunchExportIcsFileCreator(val tableId: String, val alarmMinutes: Int?) :
        ConversionEvent()
