...
/** 更新渠道信息 */
data class UpdateChannel(val id: String, val titleResId: Int, val url: String)
...
class UpdateChecker(private val context: Context) {
    ...
    companion object {
        ...
        // 默认更新渠道
        const val DEFAULT_PLATFORM_URL = GITEE_INDEX_URL

        // 可供选择的更新渠道列表
        val UPDATE_CHANNELS = listOf(
            UpdateChannel("gitee", R.string.update_channel_gitee, GITEE_INDEX_URL),
            UpdateChannel("github", R.string.update_channel_github, GITHUB_INDEX_URL)
        )
    }
    ...
    suspend fun checkUpdate(platformUrl: String): UpdateStatus = withContext(Dispatchers.Default) {
        try {
            ...
            val flavorInfo = when (currentFlavorId) {
                ...
                "dev" -> updateIndex.dev
                else -> throw IllegalStateException(context.getString(R.string.error_unknown_flavor))
            }
            ...
            val finalDownloadUrl = flavorInfo.downloadLinks[deviceAbi]
                ...
                ?: throw IllegalStateException(context.getString(R.string.error_no_suitable_abi))

            return@withContext UpdateStatus.Found(flavorInfo, finalDownloadUrl)

        } catch (e: Exception) {
            return@withContext UpdateStatus.Error(context.getString(R.string.error_update_check_failed_format, e.message ?: "未知错误"))
        }
    }
}