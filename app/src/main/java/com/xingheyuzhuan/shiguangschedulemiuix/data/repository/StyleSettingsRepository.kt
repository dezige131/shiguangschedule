...
import com.xingheyuzhuan.shiguangschedulemiuix.data.model.toCompose
...
class StyleSettingsRepository @Inject constructor(
    ...
    /** 设置是否启用悬浮底栏 */
    suspend fun setEnableFloatingBottomBar(enable: Boolean) = updateStyle {
        enableFloatingBottomBar = enable
    }

    /** 设置是否启用底栏玻璃模糊效果 */
    suspend fun setEnableBottomBarBlur(enable: Boolean) = updateStyle {
        enableBottomBarBlur = enable
    }

    /** 设置配色模式 */
    suspend fun setColorSchemeMode(mode: ColorSchemeMode) = updateStyle {
        colorSchemeMode = mode.toProto()
    }

    /** 设置 Monet 种子颜色 */
    suspend fun setMonetSeedColor(color: Long?) = updateStyle {
        if (color == null) {
            clearMonetSeedColor()
        } else {
