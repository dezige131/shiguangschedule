...
class AndroidBridge(
    ...
    private val uiEventChannel: MutableSharedFlow<WebUiEvent>,
    ...
    fun showAlert(titleText: String, contentText: String, confirmText: String, promiseId: String) {
        handler.post {
            ...
            coroutineScope.launch {
                uiEventChannel.emit(WebUiEvent.ShowAlert(data, promiseCallback))
            }
        }
    }
    ...
    fun showPrompt(
        ...
        handler.post {
            ...
            coroutineScope.launch {
                uiEventChannel.emit(WebUiEvent.ShowPrompt(data, onRequestValidation, errorFlow.asSharedFlow(), onCancel))
            }
        }
    }
    ...
    fun showSingleSelection(
        ...
        handler.post {
            try {
                ...
                coroutineScope.launch {
                    uiEventChannel.emit(WebUiEvent.ShowSingleSelection(data, promiseCallback))
                }
    ...
    /** JS 调用：将网页上抓取到的凭据传回 Android 端请求保存。 */
    @JavascriptInterface
    fun onCredentialsCaptured(username: String, password: String) {
        handler.post {
        coroutineScope.launch {
            uiEventChannel.emit(WebUiEvent.RequestSaveCredentials(username, password))
        }
        }
    }
